...
export function cloneBoard(board: ChessPiece[][]): ChessPiece[][] {
...
export function getPossibleMoves(board: ChessPiece[][], position: Position, gameState: GameState): Position[] {
  ...
  return moves.filter((move) => {
    ...
    return !isKingInCheck(newBoard, piece.color, gameState)
  })
}

// Get all possible moves for a pawn
function getPawnMoves(board: ChessPiece[][], position: Position, gameState: GameState): Position[] {
...
function getRookMoves(board: ChessPiece[][], position: Position): Position[] {
  const { row, col } = position
  const piece = board[row][col]
  if (!piece) return []
...
function getKnightMoves(board: ChessPiece[][], position: Position): Position[] {
  const { row, col } = position
  const piece = board[row][col]
  if (!piece) return []
...
function getBishopMoves(board: ChessPiece[][], position: Position): Position[] {
  const { row, col } = position
  const piece = board[row][col]
  if (!piece) return []
...
// Get all possible moves for a queen (combination of rook and bishop moves)
function getQueenMoves(board: ChessPiece[][], position: Position): Position[] {
  return [...getRookMoves(board, position), ...getBishopMoves(board, position)]
}

// Get all possible moves for a king
function getKingMoves(board: ChessPiece[][], position: Position, gameState: GameState): Position[] {
  const { row, col } = position
  const piece = board[row][col]
    ...
    if (
      ...
      if (rook && rook.type === "rook" && rook.color === piece.color && !rook.hasMoved) {
        ...
        const kingNotInCheck =
          ...
          !isPositionUnderAttack(board, { row, col: col + 1 }, piece.color, gameState)
  ...
  for (const direction of directions) {
    ...
    while (isValidPosition({ row: currentRow, col: currentCol })) {
      ...
      if (piece) {
        ...
        // Any piece (friend or foe) blocks further checking in this direction
        break
      }
...
function findKing(board: ChessPiece[][], color: Color): Position | null {
...
export function isKingInCheck(board: ChessPiece[][], color: Color, gameState: GameState): boolean {
...
export function movePiece(
  ...
  if (!piece) return { newBoard, newGameState: { ...gameState }, capturedPiece: null }

  // Store the captured piece for notation
  ...
  if (piece.type === "pawn") {
    ...
    // Clear en passant target for non-pawn moves
    newGameState.enPassantTarget = null
  }
...
export function isCheckmate(board: ChessPiece[][], color: Color, gameState: GameState): boolean {
  ...
  // No legal moves and king is in check, it's checkmate
  return true
}

// Check if the game is in stalemate
export function isStalemate(board: ChessPiece[][], color: Color, gameState: GameState): boolean {
  ...
  // No legal moves and king is not in check, it's stalemate
  return true
}
